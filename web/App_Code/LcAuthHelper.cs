...
public static class LcAuthHelper
{
    ...
    #region Signup
    const string COUNTRY_CODE_USA = "US";
    ...
    /// - countryCode [optional defaults to COUNTRY_CODE_USA]
    /// - countryID [deprecated: use countryCode]
    ...
    public static LoginResult Signup(WebPage page)
    {
        ...
        if (page.Validation.IsValid())
        {
            // ISO 2-letters CODE for the country
            var countryCode = Request.Form["countryCode"] ?? COUNTRY_CODE_USA;
            // Temporary backwards support for the countryID integer value (deprecated property, prefer countryCode):
            if (Request.Form["countryCode"].IsEmpty())
            {
                // Get user value, validate with 'Locale' so we get a default USA value if invalid
                countryCode = LcRest.Locale.GetCountryCodeByID(Request.Form["countryID"].AsInt(0));
            }
            ...
            if (LcAuth.ExistsEmail(email))
            {
                ...
                using (var db = new LcDatabase())
                {
                    ...
                    home.countryID = LcRest.Locale.GetCountryIDByCode(countryCode);
                ...
                ", profileTypeStr, firstName, lastName, email, logged.userID, countryCode, phone));
            ...
            else
            {
                ...
                var registered = LcAuth.RegisterUser(email, firstName, lastName, password,
                    isServiceProfessional, utm, -1, null, phone, null, countryCode, isOrganization);
                ...
                ", profileTypeStr, firstName, lastName, email, registered.UserID, countryCode, phone));
