...
public static class LcCalendar
{
    ...
    /// <summary>
    /// It creates a full date event 
    /// </summary>
    /// <param name="userID"></param>
    /// <param name="date"></param>
    /// <param name="AvailabilityTypeID"></param>
    /// <returns></returns>
    public static CalendarEvents CreateFullDateEventFromMonthlyCalendar(int userID, DateTime date, CalendarDll.AvailabilityTypes AvailabilityTypeID)
    {
        var newevent = new CalendarDll.Data.CalendarEvents();
        newevent.UserId = userID;
        // Availability event / appointment: 3
    ...
    public static void SaveMonthlyCalendarJsonData(int userID, dynamic data)
    {
        ...
        if (data.slots != null)
        {
            foreach (var slot in data.slots)
            {
                ...
                var availID = CalendarDll.AvailabilityTypes.UNAVAILABLE;
                if (avail == "available")
                {
                    availID = CalendarDll.AvailabilityTypes.FREE;
                }
                var date = DateTime.Parse(slot.Key);
                var ev = CreateFullDateEventFromMonthlyCalendar(userID, date, availID);
                // Add it for database
                ent.CalendarEvents.Add(ev);
            }
        }
    ...
    public static List<CalendarEvents> GetUserAppointments(int userID)
    {
        using (var ent = new loconomicsEntities())
        {
            return ent.CalendarEvents
                ...
                .Where(c => c.UserId == userID && (c.EventType == 3 || c.EventType == 5)) // && !c.Summary.StartsWith("__generated__"))
                .ToList();
        }
    }
