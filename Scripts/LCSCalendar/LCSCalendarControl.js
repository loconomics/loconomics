...
$("div.lcs_calendar_control").each(function (index, curDiv) {
    ...
    if (CalendarContainer) {
        ...
        GetData(LCSService_url, LCSAuthtoken, LCSContainerRef, LCSDisplayType);  //make ajax call
    }
    else {
...
function GetData(service_url, authtoken, containerRef, displayType) {
    //alert("getting data");
    var jqxhr = $.ajax({ url: service_url
            ...
            .complete(function (data) { onLCSCalendar_Complete(data); });
...
// Method called when the Ajax request is compete (after error or success)
function onLCSCalendar_Complete(data) {
    //alert("complete2");
}
...
function onLCSCalendar_Success(data, containerRef, displayType) {
    ...
    var today = new Date();
    var month = today.getMonth() + 1; //javascript returns months as 0-11
    var year = today.getFullYear(); //4 digit year
    var daydate = today.getDate(); //returns day of current month 1-31
    var TodaySortedDate = year + "-" + month + "-" + daydate;
   ...
   $("div.LCSCalendar_EventDay").hover(
    ...
    //*************************************
    //
    //  TODO (1): Make set current day crossbrowser compliant (Jordan, could use some ideas here)
    //
    //  current implementation of setting the current day and hi-lited and showing
    //  the corresponding day card on initial load, causes errors in IE
    //*************************************
         ...
         var arHourIndex = new Array(HourIndexSize);
            try {
                if (!jsonObj.Events) {
                    ...
                    //Loop through the events and convert the times to 30 min time blocks on a 24hr clock

                    jsonObj.MyEvents = [];

                    //*****************************************************************************************************
                    //
                    //  TODO (2): Fix logic (Pat's working on this)
                    //
                    // this logic needs to be revisited, (scrapped and redone)
                    // there is currently more work than needed being done, for instance the push is not longer necessary
                    // primarily, there is a rounding error which is making some event off by one time segment
                    //*****************************************************************************************************
                ...
                $("#test").empty(); //remove any detail placed here by last click event
                $("#Event_tmpl").tmpl(jsonObj).appendTo("#test");
                ...
                $('.LCSCalTimeSegment[title="synched event"]').addClass("LCSCAL_SetBusy");
...
function InsertTable(container, className) {
    var tablestr = "<table class='" + className + "'><tr><td id='" + container + "_tablecell' ></td><tr><td id='" + container + "_log'></td></table>";
