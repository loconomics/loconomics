...
/**
 * Moves navigation away the webapp, forcing a browser page load to the given
 * address.
 * Links to full URLs/out of domain are supported out of the box by the Shell,
 * but the purpose of this is to support same domain URLs that are not managed
 * by this webapp/shell, like the public pages generated by loconomics-pages.
 * @param {string} to URL path out of the webapp
 */
function goOutWebapp(to) {
    return class Redirect extends Activity {
        static get template() { return ''; }

        show() {
            setTimeout(() => window.location = to, 10);
        }
    };
}
